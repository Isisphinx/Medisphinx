...
const SetFakeList = () => {
  ...
  const createUser = () => ({ uuid: faker.random.uuid(), email: faker.internet.email(), address: faker.address.streetAddress(), bio: faker.lorem.sentence(), image: faker.image.avatar(), name: faker.name.findName(),
                              age: faker.random.number({'min': 0, 'max':80}), years: faker.date.between('2020-12-12', '2022-02-02').getFullYear(), days: ((faker.date.between('2020-12-12', '2022-02-02').getDate()) + 1), month: ((faker.date.between('2020-12-12', '2022-02-02').getMonth()) + 1)})
